...
import { commonFetchOptions, getPortalId } from '../common/common'
import _ from 'lodash'

export default async (id, type = 'CONTACT') => {
  const url = `${host}/api-passthrough/graphql/crm?portalId=${getPortalId()}&clienttimeout=15000`
  const opts = commonFetchOptions()
  const data = {
  ...
  const res = await fetch.post(url, data, opts)
